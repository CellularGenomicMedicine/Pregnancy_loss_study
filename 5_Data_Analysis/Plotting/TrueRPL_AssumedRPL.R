...
###Position = "dodge"
{
RPL_types_plot <- ggplot(data = subset(RPL_types_SNPHapla, !is.na(Seg_origin2)), aes(x = RPL_status_based_on_Livebornchild, fill = factor(Seg_origin2, levels = Seg_Or_Order))) +
  geom_bar(stat="count", color =  "black", width=0.7, position = position_dodge2(preserve = "single")) +
  scale_fill_manual(values = c("Meiotic" = "#048F45", "Mitotic" = "#0CB702")) +
