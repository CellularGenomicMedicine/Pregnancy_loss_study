callpcfBAF_mod <- function(BAFph,gammaSC,gammaMC,plateau,SibPattern){
  ...
  for(i in 4:ncol(BAFph)){
    ...
    for(chr in unique(BAFph[,"Chr"])){
      ...
      SegChr <- res$yhat},error=function(e){print(paste("Chr",chr,"is not segmented"))})
      if(TC==paste("Chr",chr,"is not segmented")){SegChr<-logRChr}
