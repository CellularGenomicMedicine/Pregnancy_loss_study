###########################################################################################################################
# Author: Rick Essers (Adapted from haplarithmisis scripts by Masoud Zamani Esteki)
# Lab: Cellular Genomic Medicine, Clinical Genetics, Maastricht University Medical Center (MUMC+)

# script purpose: To apply haplarithmisis to individual pregnancy loss families. 
#                 Haplarithmisis is a method that determines haplotypes, as well as the copy number and segregation origin of these haplotypes across the genome. 
#                 The method advances and facilitates the detection of genuine DNA copy-number (or copy-neutral) aberrations and also reveals their parental and mechanistic origin.

# input: .adj file containing SNP genotyping data for each individual pregnancy loss family. 

# output: Haplarithm plots containing BAF, LogR, and haplotyping information on the fetal DNA samples (Extraembryonic mesoderm and chorionic villi) and BAF and LogR of both parents. 
#         These plots show; copy number and copy neutral aberrations (CNV of >100kb, chromosomal aberrations, genome wide aberrations), 
#         parental (paternal or maternal) origin of the aberration, mechanistic (meiotic I, meitoic II, or mitotic) origin of the aberration, 
#         degree of mosaicism (range of 10%), and maternal cell contamination. 
...
### use args when using a loop 
args <- commandArgs(TRUE)
Family <- args[1]

###use this when running inidividual family
Family       <- "PL2726.adj"

script       <- "/Projects/PregnancyLoss/Scripts/Haplarithmisis/Haplarithmisis_RawScripts/"
analyses     <- "/Projects/PregnancyLoss/Scripts/Haplarithmisis/Haplarithmisis_RawScripts/"
logs         <- "/Projects/PregnancyLoss/Scripts/Haplarithmisis/Haplarithmisis_RawScripts/logs/"
SibPattern   <- "E*_Bl*"
Seed         <- "Affected"
gtype_window <- 10000

outPathInit <- "/Projects/PregnancyLoss/Data/Output"
...
parametersFile <- paste0("/Projects/PregnancyLoss/Scripts/Haplarithmisis/Haplarithmisis_RawScripts/Codes/","Generic_Parameters.txt")
IntervalFile <- paste0("/Projects/PregnancyLoss/Scripts/Haplarithmisis/Haplarithmisis_RawScripts/Codes/","Generic_Intervals.txt")
...
genomebafplot_PLstudy(BAFs,logRs,SegLogRs ,PhBAF,ChrsLength,ideogram,Family,outPathGenome)
...
#Chroms <- "5"
chrspecbafplot_PLstudy(Haps[["dataHap"]],Haps[["dataHapRaw"]],dataPo,BAFs,logRs,SegLogRs,PhBAF,ChrsLength,ideogram,Family,outPathChr,Chroms,Int)
